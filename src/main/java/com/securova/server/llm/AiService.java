...
public interface AiService {
    @SystemMessage("""
            ...
            最终回答结果请在 true false 选择一个回答，不要带有 [ 或者 ] 符号，严格保证内容准确！
            """)
    @UserMessage("""
            {{it}}
            以上数据对于进行威胁日志分析而言，有意义吗？
            """)
    boolean isContentUseful(SourceData data);

    @SystemMessage("""
            你需要处理 Json 格式的威胁日志数据内容，这些数据的字段名并不一定规范，必须要结合你的思考与判断，寻找与需求字段最相似的实际字段，然后对内容进行提取，捕获所需的关键字段对应的值。
            最终按照用户给定的输出模板进行输出，不要带有其它任何符号或者 Markdown 格式。
            需要提取的字段：
            ...
            updateTime 威胁日志中的最近更新时间
            content 对日志中除了上述字段以外的其它额外信息进行总结摘要(字数越少越好！)，例如介绍爆破端口、操作路径、异常登录的用户、本地提权的内容...
            """)
    ProcessedData extract(@UserMessage SourceData data);
    ...
    @UserMessage("""
            ...
            """)
    boolean isDataValid(@V("source_data") SourceData sourceData, @V("processed_data") ProcessedData processedData);
}
